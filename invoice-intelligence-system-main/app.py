...
selected_model = st.sidebar.selectbox("Choose a model to use:", ["Freight Cost Prediction", "Invoice Risk Flagging"])
...
if selected_model == "Freight Cost Prediction":
    ...
    **Objective:**
    ...
    with st.form("freight_form"):
        ...
        with col2:
            dollars = st.number_input("💵Invoice Dollars", min_value=1.0, value=15000.0)
    ...
    **Objective:**
    ...
    with st.form("invoice_flag_form"):
        ...
        with col2:
            invoice_dollars = st.number_input("💵Invoice Dollars", min_value=1.0, value=500.0)
            total_item_quantity = st.number_input("📦Total Item Quantity", min_value=1, value=150)
        with col3:
            total_item_dollars = st.number_input("💰Total Item Dollars", min_value=1.0, value=2450.0)
    ...
    if submit_flag:
        ...
        if is_flagged:
            st.error("⚠️ This invoice is predicted to be **flagged for manual approval**.")
        else:
            st.success("✅ This invoice is predicted to be **approved automatically**.")